    ...
    def CATEGORY(self):
        ...
        for i in category:
            ...
            if i['key'] == 'comedy':
                ...
                xbmc.log('*************** КАТЕГОРИЯ::::::: *********************** ' + url, xbmc.LOGDEBUG)
...
# Количество страниц в категории ******************
    ...
    def get_total_page(self):
        ...
        cat = sys.argv[2].split('=')[1] + '/'
        r = requests.get(url_Main + cat, headers=headers)
        soup = BeautifulSoup(r.content, 'html.parser', parse_only=SoupStrainer('div', class_='navigation'))
...
#***************************************///////////////////////////////////////////////////****************************************    
    ...
    def films_test(self):
        ...
        #u = re.compile('page')
        #u2 = u.findall(url)
        xbmc.log('******* ::::::::::::: Формирование URL = 1 ::::::::::: ******* ' + urllib.parse.unquote_plus(url), xbmc.LOGDEBUG)
                

        
        for i in range (1, self.get_total_page()):        
                
            try:
                if url == url_Main + cat:
                   url = url_Main + cat + 'page/' + str(i) + '/'
                else:
                   pass
            except:
                pass
                
        xbmc.log('******* ::::::::::::: Формирование URL = 2 ::::::::::: ******* ' + url, xbmc.LOGDEBUG)        
        ...
        xbmc.log('******* ::::::::::::: Формирование URL = 3 ::::::::::: ******* ' + url, xbmc.LOGDEBUG)     
        for i in db_films:
            self.addDir(i['title'], i['url'], 0, 'DefaultVideo.png')
            xbmc.log('******* ::::::::::::: Формирование URL = 4 ::::::::::: ******* ' + url, xbmc.LOGDEBUG) 
            
        #self.addLink(i['title'], i['url'], 'DefaultVideo.png')
        
        self.addDir('Next PAGE', 'plugin://plugin.parser.tor/'+'?url='+urllib.parse.unquote_plus('http://filmitorrent.biz/boevik/page/2/'), 1, 'DefaultVideo.png')
        xbmcplugin.endOfDirectory(addon_handle)
        

# Нужно сформировать каким-то образом адрес URL ************************************
    
    def addDir(self, title, url, mode, iconimage):
        
        ok = True
        liz = xbmcgui.ListItem(title)
        ok = xbmcplugin.addDirectoryItem(addon_handle, url, listitem=liz, isFolder=True) #*****************????????????????? правильный адрес ?????знак ? и =
        #xbmcgui.Dialog().ok('Warning!', url) 
        xbmc.log('******* ::::::::::::: Формирование addDIR ::::::::::: ******* ' + url, xbmc.LOGDEBUG)
        return ok
