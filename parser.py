...
def main_CAT():
    ...
    for l in db_main_cat:
        add_CAT(l['title'], url_Main + cat, l['index'], 'tag', 'DefaultVideo.png')
    xbmcplugin.endOfDirectory(addon_handle)
    #xbmcgui.Dialog().ok('Function MAIN!', str(mode))

def sub_CAT(indx_1, tag_link):
    if DEBUG:
        log('*+*+*+*+*+*+*+*+*+*+*+*+*+*+* sub_CAT() ===== ' + str(mode))
    soup = BeautifulSoup(request(cat), 'html.parser', parse_only=SoupStrainer('div', id='content'))
...
def video_link():
    ...
    xbmcgui.Dialog().ok('video_link MODE!', str(mode))
                                #'search/0/1/010/0/'
    ...
    for i in db_mov_lnk:
        add_CAT(i['title'], url_Main + '/search/' + str(page) + part_url[0] + str(mode[0]).split('=')[1] + '/', 9, 'tag', 'DefaultVideo.png')
                                      # search/ + page/ + /1/010/0/
    ...
    add_CAT('Next Page', 'http://222njbta.ihg.xrutor.org/search/1/1/010/0/Арт-хаус', 10, 'tag', 'DefaultVideo.png')
    xbmcplugin.endOfDirectory(addon_handle)
...
def add_CAT(title, url, mode, part_url, iconimage):
    ...
    #xbmcgui.Dialog().ok('Warning!', str(sys.argv[2].split('=')))
    #xbmc.log('******* ::::::::::::: MODE ::::::::::: ******* ' + str(mode), xbmc.LOGDEBUG)
    log('******* ::::::::::::: addCatt ::::::::::: ******* ' + urllib.parse.unquote(str(sys.argv[2].split('=')[2])))
    return ok
...
xbmcgui.Dialog().ok('Function IF!', str(mode))
if mode == None:
    pass
elif mode[0] == '2':      # tag = search/0/1/010/0/:      search/  +  page/ +  1/010/0/
    log('******* ///////////////////// West FILMS ::::::::::: *******')
    sub_CAT(2, '/1/010/0/')
elif mode[0] == '3':           # tag = search/ 0 /5/010/0/
    log('******* ///////////////////// USSR FILMS ::::::::::: *******')
    sub_CAT(3, '/5/010/0/')
elif mode[0] == '4':    # tag = search/ 0/ 4/0/0/
    log('******* ///////////////////// SERIAL WeST FILMS ::::::::::: *******')
    sub_CAT(4, '/4/0/0/')
elif mode[0] == '5':          # tag = search/ 0/ 16/0/0/
    tag_link = 'search/0/16/0/0/'
    sub_CAT(5, '/16/010/0/')
elif mode[0] == '6':             # tag = search/ 0/ 6/0/0/
    tag_link = 'search/0/6/0/0/'
    sub_CAT(6)
elif mode[0] == '7':        # tag = tag/7/
    tag_link = 'tag/7/'
    sub_CAT(7)
elif mode[0] == '8':                 # tag = search/ 0/ 10/010/0/
    sub_CAT(8, '/10/010/0/')

#elif urllib.parse.unquote(str(sys.argv[2].split('=')[2])):
#    tag_part = 'tag/1/'
#    video_link()
...
# Нужно сделать сверку категории (жанра) фильма и последующую загрузку списка фильмов
